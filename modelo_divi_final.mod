...
# Parâmetros
# Capacidades por nível de cuidado
param C1{P, L[1]};    
param C2{L[2]}; 
param C3{L[3]}; 

# Custos das equipes por nível de cuidado
param CE1{c1 in E[1]};   
param CE2{c2 in E[2]};
param CE3{c3 in E[3]};

# Custos fixos para abertura de unidades por nível
param FC1{L[1]};   
param FC2{L[2]}; 
param FC3{L[3]}; 

# Custos variáveis por paciente por nível
param VC1{P, L[1]};   
param VC2{L[2]};
param VC3{L[3]};

# População por ponto de demanda
param W{I, P};  

# Requisitos mínimos por equipe
param MS1{E[1]};   
param MS2{E[2]};
param MS3{E[3]};

# Equipes existentes nas unidades
param CNES1{E[1], L[1]}; 
param CNES2{E[2], L[2]};
param CNES3{E[3], L[3]};

#Número de unidade por nível
param U{K}; 

# Proporção de pacientes entre níveis
param O1{L[1]};    
param O2{L[2]};   

# Distâncias entre unidades por nível
param D1{I,L[1]};
param D2{L[1], L[2]};
param D3{L[2], L[3]};
...
var y{i in I, j1 in L[1]}, >=0, binary;
var y1{j1 in L[1]}, >=0, binary;
var y2{j2 in L[2]}, >=0, binary;
var y3{j3 in L[3]}, >=0, binary;
...
display sum{p in P, i in I, j1 in L[1]}u1[p,i,j1]; 
display sum{j1 in L[1], j2 in L[2]}u2[j1,j2];
display sum{j2 in L[2], j3 in L[3]}u3[j2,j3];
